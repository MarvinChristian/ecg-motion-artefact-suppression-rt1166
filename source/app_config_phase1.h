 ...
 *        main_phase1.c
 *        drivers/ecg_adc.c      (uses APP_ECG_ADC_CH_OUT/REFOUT,
 *                                APP_ADC_RESULT_SHIFT)
 *        drivers/imu_manager.c  (uses APP_IMU_SPI_SRC_CLOCK_HZ, APP_IMU_SPI_BAUD_HZ)
 *        drivers/ads1293.c      (uses APP_ADS1293_SPI_* via main_phase1.c)
 */
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Conversion: ecg_mV = ecg_corr * (1800 / 4096).

   Channel mapping:
     ch0 (APP_ECG_ADC_CH_OUT)     — AD8233 OUT terminal
     ch1 (APP_ECG_ADC_CH_REFOUT)  — AD8233 REFOUT/reference terminal

   Firmware computes ecg_corr = out_raw - refout_raw.
   ═══════════════════════════════════════════════════════════════════════════ */
...
/*
 * ADC channel decision traceability:
 *   empirical: existing recordings used LPADC1 A1_0/A1_1 successfully.
 *   empirical: the other labelled eval-board filter taps were not active in
 *              the current hardware setup, so they are not sampled.
 *
 * Firmware computes:
 *
...
/* ═══════════════════════════════════════════════════════════════════════════
   ECG SAMPLING RATE
   ═══════════════════════════════════════════════════════════════════════════
   500 Hz is the nominal ADC tick rate. The effective printed sample rate is
   determined by UART payload length and must be measured from t_us timestamps
   after every serial-format change.
...
/* ADS1293 shares LPSPI1 with the IMUs but uses SPI mode 0. The driver
   switches CPOL/CPHA per transfer; the baud rate remains 1 MHz to match the
   working Protocentral Arduino configuration. */
