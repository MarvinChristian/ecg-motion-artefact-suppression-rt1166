...
% TITLE:    Phase 2+3 — Unified ECG Filter + MAS Analysis GUI
% DATE:     19/04/2026
%
% SUMMARY:
%   Extends phase2_analyzer with MAS evaluation (M1-M6).
...
%% ═══════════════════════════════════════════════════════════════════════
    ...
    function update_mas_tab(sig_in, filtered, after_mas, ref_display, ref_disp_lbl, ...
        ...
        if ~isempty(imu_ref)
            coh_win = max(min(512, floor(N_coh/4)), 64);
            [cxy_b, f_coh] = mscohere(mas_input(1:N_coh),      imu_ref, hamming(coh_win), floor(coh_win/2), [], Fs);
            [cxy_a, ~    ] = mscohere(mas_direct_out(1:N_coh), imu_ref, hamming(coh_win), floor(coh_win/2), [], Fs);
        ...
        mlines{end+1,1} = '  ② IMU–ECG COHERENCE  (0.5–8 Hz, γ²)';
        mlines{end+1,1} = repmat('·',1,65);
        if isnan(coh_before)
...
function selected = rt_select_bands_by_coherence(d, ref_bank, Fs, cal_sec, max_lag_ms, peak_thr, max_bands, bw)
    ...
    nper = min(512, max(64, floor(calN/2)));
    nper = min(nper, calN);
