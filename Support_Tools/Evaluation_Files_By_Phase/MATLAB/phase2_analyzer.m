% phase2_analyzer.m
%
% Phase 2/3 ECG filter + MAS analysis GUI. Loads a recording, applies a
% selected BPF (B1-B8) + notch (N1-N5) + MAS combination, and shows time
% and frequency views with summary metrics. Legacy M1-M31 MAS family lives
% in analyzer_history.m; coherence/Wiener diagnostics live in
% signal_diagnose_gui.m.
%
% MAS parameters:
%   NLMS    mu=0.50, step cap=0.001, leak=0.999995 (firmware-matched)
%   RLS     lambda=0.999
%   APA     mu=0.10, order=2
...
%   2/4   : ECG_ONLY / ECG_ONLY_DEBUG (Phase 2 only, no MAS)
%   17    : ECG_IMU short
%   20    : ECG_IMU full
%   21    : ADS1293_IMU (CH1, CH2, 3xIMU)
%   22    : ECG_IMU + AD8233 OUT/REFOUT (debug taps ignored)
%   23    : ECG_IMU + ADS1293 extras
%   24    : ADS1293_IMU_TS (adds dt0/1/2_us)
%
% Depends on: apply_biquad.m, apply_notch.m
...
MAS_NAMES = {
    ...
    'M24: RT source coherent-band NLMS',
    'M25: RT staged active-electrode NLMS',
    'M26: RT source relaxed coherent-band',
    'M27: Aggressive all-IMU NLMS',
    'M28: Aggressive all-IMU bands',
    'M29: AD8233 OUT-matched all-IMU',
    'M30: Adaptive event-band all-IMU',
    'M31: Validated adaptive source event-band'
};
MAS_LIST_IMPL  = [ ...
    0, ...
    32, 33, 35, 36, 37, 38, ...
...
MAS_LIST_NAMES = {
    ...
    'LMS accel xyz',
    'LMS gyro xyz',
    'LMS accel+gyro 6-axis',
    'NLMS |a| magnitude',
    'NLMS |g| magnitude',
    'NLMS |a|+|g| magnitudes',
    'NLMS accel xyz',
    'NLMS gyro xyz',
    'NLMS accel+gyro 6-axis',
    'RLS |a| magnitude',
    'RLS |g| magnitude',
    'RLS |a|+|g| magnitudes',
    'RLS accel xyz',
    'RLS gyro xyz',
    'RLS accel+gyro 6-axis',
    'SS-LMS |a| magnitude',
    'SS-LMS |g| magnitude',
    'SS-LMS |a|+|g| magnitudes',
    'SS-LMS accel xyz',
    'SS-LMS gyro xyz',
    'SS-LMS accel+gyro 6-axis',
    'VS-NLMS |a| magnitude',
    'VS-NLMS |g| magnitude',
    'VS-NLMS |a|+|g| magnitudes',
    'VS-NLMS accel xyz',
    'VS-NLMS gyro xyz',
    'VS-NLMS accel+gyro 6-axis'
};
MAS_SOURCE_NAMES = {
    'IMU0 - LL',
    'IMU1 - LA',
...
uilabel(sidebar, 'Text','ECG Filter + MAS Analyser  (B1-B8 | N1-N5 | LMS/NLMS/RLS)',...
...
bpf_btn(9).Value = true;   % B8 default: Butterworth 12th, 0.5-40 Hz
...
notch_btn(4).Value = true;  % N3 default: fixed 50 Hz IIR + adaptive NLMS residual
...
mas_list = uilistbox(sidebar, ...
    ...
    'Position',[10 286 320 64], ...
    'Value','None (skip MAS)', ...
    'FontSize',8.5,'FontColor',[0.82 0.82 0.82], ...
    'BackgroundColor',[0.13 0.13 0.16], ...
    'Enable','off');

uilabel(sidebar,'Text','MAS SOURCE', ...
    'Position',[10 266 320 16], ...
    'FontSize',8.5,'FontWeight','bold','FontColor',[0.55 0.75 0.95]);

mas_source_dd = uidropdown(sidebar, ...
    'Items', MAS_SOURCE_NAMES, ...
    'Value','Lead-matched RA-pair', ...
    'Position',[10 240 320 22], ...
    'FontSize',8.5,'BackgroundColor',[0.13 0.13 0.16], ...
    'FontColor',[0.85 0.85 0.85], ...
    'Enable','off');

uilabel(sidebar,'Text','MAS TUNING', ...
    'Position',[10 216 320 16], ...
    'FontSize',8.5,'FontWeight','bold','FontColor',[0.55 0.75 0.95]);

uilabel(sidebar,'Text','Taps', 'Position',[10 198 52 14], ...
    'FontSize',7.5,'FontColor',[0.68 0.68 0.70]);
mas_taps_edit = uieditfield(sidebar,'numeric', ...
    'Value',32,'Limits',[1 64], ...
    'Position',[10 178 52 20], ...
    'FontSize',8,'BackgroundColor',[0.13 0.13 0.16], ...
    'FontColor',[0.85 0.85 0.85], ...
    'Tooltip','Tap depth per reference signal. Current firmware default is 32.', ...
    'Enable','off');

mas_band_defaults = [0.5 1.5; 10.0 17.0; 28.0 32.0];
mas_band_lo_edit = cell(1, 3);
mas_band_hi_edit = cell(1, 3);
mas_band_label_x = [62 104 146 188 230 272];
for bb = 1:3
    uilabel(sidebar,'Text',sprintf('B%d lo', bb), 'Position',[mas_band_label_x(2*bb-1) 198 38 14], ...
        'FontSize',7.5,'FontColor',[0.68 0.68 0.70]);
    mas_band_lo_edit{bb} = uieditfield(sidebar,'numeric', ...
        'Value',mas_band_defaults(bb, 1),'Limits',[0.05 40], ...
        'ValueDisplayFormat','%.3g', ...
        'Position',[mas_band_label_x(2*bb-1) 178 38 20], ...
        'FontSize',8,'BackgroundColor',[0.13 0.13 0.16], ...
        'FontColor',[0.85 0.85 0.85], ...
        'Tooltip','Low cutoff for a transport-motion IMU reference band.', ...
        'Enable','off');

    uilabel(sidebar,'Text',sprintf('B%d hi', bb), 'Position',[mas_band_label_x(2*bb) 198 38 14], ...
        'FontSize',7.5,'FontColor',[0.68 0.68 0.70]);
    mas_band_hi_edit{bb} = uieditfield(sidebar,'numeric', ...
        'Value',mas_band_defaults(bb, 2),'Limits',[0.30 40], ...
        'ValueDisplayFormat','%.3g', ...
        'Position',[mas_band_label_x(2*bb) 178 38 20], ...
        'FontSize',8,'BackgroundColor',[0.13 0.13 0.16], ...
        'FontColor',[0.85 0.85 0.85], ...
        'Tooltip','High cutoff for a transport-motion IMU reference band.', ...
        'Enable','off');
end

uilabel(sidebar,'Text','LMS cap', 'Position',[10 164 58 14], ...
    'FontSize',7.5,'FontColor',[0.68 0.68 0.70]);
mas_lms_cap_edit = uieditfield(sidebar,'numeric', ...
    'Value',0.01,'Limits',[1e-7 0.1], ...
    'ValueDisplayFormat','%.4g', ...
    'Position',[10 144 58 20], ...
    'FontSize',8,'BackgroundColor',[0.13 0.13 0.16], ...
    'FontColor',[0.85 0.85 0.85], ...
    'Tooltip','Plain LMS upper step cap. Actual step is min(cap, 0.05/(refs*taps)).', ...
    'Enable','off');

uilabel(sidebar,'Text','NLMS mu', 'Position',[78 164 58 14], ...
    'FontSize',7.5,'FontColor',[0.68 0.68 0.70]);
mas_nlms_mu_edit = uieditfield(sidebar,'numeric', ...
    'Value',0.50,'Limits',[1e-5 2.0], ...
    'ValueDisplayFormat','%.4g', ...
    'Position',[78 144 58 20], ...
    'FontSize',8,'BackgroundColor',[0.13 0.13 0.16], ...
    'FontColor',[0.85 0.85 0.85], ...
    'Tooltip','NLMS base mu before instantaneous power normalization.', ...
    'Enable','off');

uilabel(sidebar,'Text','Step cap', 'Position',[146 164 58 14], ...
    'FontSize',7.5,'FontColor',[0.68 0.68 0.70]);
mas_nlms_cap_edit = uieditfield(sidebar,'numeric', ...
    'Value',0.001,'Limits',[1e-5 0.5], ...
    'ValueDisplayFormat','%.4g', ...
    'Position',[146 144 58 20], ...
    'FontSize',8,'BackgroundColor',[0.13 0.13 0.16], ...
    'FontColor',[0.85 0.85 0.85], ...
    'Tooltip','Per-sample NLMS update cap; lower values reduce over-adaptation.', ...
    'Enable','off');

uilabel(sidebar,'Text','RLS lam', 'Position',[214 164 54 14], ...
    'FontSize',7.5,'FontColor',[0.68 0.68 0.70]);
mas_rls_lambda_edit = uieditfield(sidebar,'numeric', ...
    'Value',0.999,'Limits',[0.900 0.99999], ...
    'ValueDisplayFormat','%.5g', ...
    'Position',[214 144 54 20], ...
    'FontSize',8,'BackgroundColor',[0.13 0.13 0.16], ...
    'FontColor',[0.85 0.85 0.85], ...
    'Tooltip','RLS forgetting factor. Higher is slower and more stable.', ...
    'Enable','off');

mas_status_lbl = uilabel(sidebar,'Text','Load an ECG_IMU file to enable MAS.', ...
    'Position',[10 124 320 16], ...
    'FontSize',8,'FontColor',[0.55 0.55 0.55]);

divider_line(sidebar, 116, 320);

% ── FILTER ORDER ──────────────────────────────────────────────────────
uilabel(sidebar,'Text','FILTER ORDER', ...
    'Position',[10 100 320 16], ...
    'FontSize',9,'FontWeight','bold','FontColor',[0.55 0.75 0.95]);
...
ax_beat = uiaxes(tab_quant,'Position',[10 610 1350 320], ...
...
quant_txt = uitextarea(tab_quant,'Position',[10 10 1350 580], ...
...
%% ----
    ...
    function load_file()
        ...
        if n_cols ~= 2 && n_cols ~= 4 && n_cols ~= 17 && n_cols ~= 20 && n_cols ~= 21 && n_cols ~= 22 && n_cols ~= 23 && n_cols ~= 24
            status_lbl.Text = sprintf('Unexpected columns: %d (expect 2, 4, 17, 20, 21, 22, 23, or 24).', n_cols);
            status_lbl.FontColor = [0.9 0.4 0.4]; return;
        end
        ...
        ads1293_24 = (n_cols == 24);
        ads1293_23 = (n_cols == 23);
        state.ch1_mV = [];
        state.ch2_mV = [];

        if ads1293_21 || ads1293_24
        ...
        if ads1293_21 || ads1293_24 || ads1293_23
            ch_dd.Enable = 'on';
        ...
        if n_cols == 17 || n_cols == 20
            ...
            mas_list.Enable = 'on';
            if strcmp(mas_list.Value, 'None (skip MAS)')
                mas_list.Value = 'NLMS accel+gyro 6-axis';
            end
            mas_source_dd.Enable = 'on';
            set_mas_tuning_enabled(true);
            if ads1293_21 || ads1293_24 || ads1293_23
                mas_status_lbl.Text = sprintf('ADS1293_IMU (%d cols) - MAS enabled. Choose |a|/|g| and IMU source. %s active.', ...
                    original_n_cols, ch_dd.Value);
            elseif original_n_cols == 17
                mas_status_lbl.Text = 'ECG_IMU (17 cols) - MAS enabled. IMU2 gyro is zero-filled, so |g| on RA will bypass.';
            else
                mas_status_lbl.Text = 'ECG_IMU (20 cols) - MAS enabled. Choose |a|/|g| and IMU source.';
            end
            ...
            mas_source_dd.Enable = 'off';
            set_mas_tuning_enabled(false);
            mas_status_lbl.Text = 'ECG_ONLY - load 17/20/21/22/23/24-col ECG_IMU/ADS1293 file for MAS.';
    ...
    function parse_imu(data, Fs)
        ...
        % Causal gravity/bias removal. The DC blocker is initialized from the
        % first sample and then uses only current/past samples, so the MAS
        % reference path matches a streamable implementation.
        accel_ac = @(x) imu_remove_dc_and_startup_windup(x, Fs, 0.75);
        gyro_ac  = @(x) imu_remove_dc_and_startup_windup(x, Fs, 50.0);

        state.imu.ax0_ac = accel_ac(ax0); state.imu.ay0_ac = accel_ac(ay0); state.imu.az0_ac = accel_ac(az0);
        state.imu.gx0_ac = gyro_ac(gx0);  state.imu.gy0_ac = gyro_ac(gy0);  state.imu.gz0_ac = gyro_ac(gz0);
        state.imu.ax1_ac = accel_ac(ax1); state.imu.ay1_ac = accel_ac(ay1); state.imu.az1_ac = accel_ac(az1);
        state.imu.gx1_ac = gyro_ac(gx1);  state.imu.gy1_ac = gyro_ac(gy1);  state.imu.gz1_ac = gyro_ac(gz1);
        state.imu.ax2_ac = accel_ac(ax2); state.imu.ay2_ac = accel_ac(ay2); state.imu.az2_ac = accel_ac(az2);
        state.imu.gx2_ac = gyro_ac(gx2);  state.imu.gy2_ac = gyro_ac(gy2);  state.imu.gz2_ac = gyro_ac(gz2);
        ...
        dt = 1 / Fs;
        vx0 = imu_remove_dc_and_startup_windup(cumtrapz(state.imu.ax0_ac) * dt, Fs, 0.10);
        vy0 = imu_remove_dc_and_startup_windup(cumtrapz(state.imu.ay0_ac) * dt, Fs, 0.10);
        vz0 = imu_remove_dc_and_startup_windup(cumtrapz(state.imu.az0_ac) * dt, Fs, 0.10);
        state.imu.vel0_ac = sqrt(vx0.^2 + vy0.^2 + vz0.^2);
    end

    function y = imu_remove_dc_and_startup_windup(x, Fs, startup_floor) %#ok<INUSD>
        x = double(x(:));
        N = numel(x);
        if N == 0
            y = x;
    ...
    function set_mas_tuning_enabled(tf)
        en = 'off';
        if tf
            en = 'on';
        end
        ctrls = [mas_taps_edit, [mas_band_lo_edit{:}], [mas_band_hi_edit{:}], ...
                 mas_lms_cap_edit, mas_nlms_mu_edit, ...
                 mas_nlms_cap_edit, mas_rls_lambda_edit];
        for cc = 1:numel(ctrls)
            ctrls(cc).Enable = en;
        end
    end

    function y = apply_selected_notch(x, notch_idx, Fs, bpf_idx, sos_bpf_data, ref_override) %#ok<INUSD>
        if notch_idx <= 0
            y = x;
            return;
        end
    ...
    function do_evaluation(bpf_idx, notch_idx, mas_idx)
        ...
        has_imu = ~isempty(state.imu.mag0_ac) || ~isempty(state.imu.mag1_ac) || ~isempty(state.imu.mag2_ac);
        do_mas = mas_idx > 0 && strcmp(state.mode,'ECG_IMU') && has_imu;
        mas_in    = [];
        ...
        sig_in         = finite_trace(sig_in, []);
        after_bpf      = finite_trace(after_bpf, sig_in);
        filtered       = finite_trace(filtered, after_bpf);
        after_notch    = filtered;
        after_mas      = finite_trace(after_mas, filtered);
        mas_input      = finite_trace(mas_input, filtered);
        mas_direct_out = finite_trace(mas_direct_out, mas_input);
        for pp = 1:numel(pipe_sigs)
            pipe_sigs{pp} = finite_trace(pipe_sigs{pp}, filtered);
        end
        ...
        finite_raw = isfinite(sig_in);
        finite_final = isfinite(final_sig);
        valid_pct = 100 * nnz(finite_raw & finite_final) / max(numel(sig_in), 1);
        if numel(t_s_data) >= 3
            dt_s = diff(t_s_data(:));
            dt_med_s = median(dt_s, 'omitnan');
            dt_jitter_us = 1e6 * median(abs(dt_s - dt_med_s), 'omitnan');
        else
            dt_jitter_us = NaN;
        end

        raw_rms = rms(sig_in(finite_raw));
        final_rms = rms(final_sig(finite_final));
        raw_p2p = robust_range(sig_in, 5, 95);
        final_p2p = robust_range(final_sig, 5, 95);
        rms_ratio = final_rms / max(raw_rms, eps);
        p2p_ratio = final_p2p / max(raw_p2p, eps);

        wide_hi_hz   = max(f_pw);
        bp_motion_db = band_reduction_db(Praw, Pmas, f_pw, 0.5, min(40.0, 0.45*Fs));
        bp_wide_db   = band_reduction_db(Praw, Pmas, f_pw, 0.5, wide_hi_hz);
        bp_mains_db  = band_reduction_db(Praw, Pmas, f_pw, 48.0, 52.0);
        bp_hf_db     = band_reduction_db(Praw, Pmas, f_pw, 35.0, min(100.0, 0.45*Fs));

        preserve = overhead_preservation_metrics(sig_in, final_sig, Fs);

        overhead_warns = 0;
        if valid_pct < 99.0 || (~isnan(dt_jitter_us) && dt_jitter_us > 0.25 * 1e6/Fs)
            overhead_warns = overhead_warns + 1;
        end
        if rms_ratio < 0.50 || rms_ratio > 1.80 || p2p_ratio < 0.50 || p2p_ratio > 1.80
            overhead_warns = overhead_warns + 1;
        end
        if preserve.ratio < 0.80 || preserve.ratio > 1.20 || preserve.qrs_amp_ratio < 0.70 || preserve.qrs_amp_ratio > 1.40
            overhead_warns = overhead_warns + 1;
        end
        if preserve.t_prd > 35 || preserve.t_amp_ratio < 0.60 || preserve.t_amp_ratio > 1.50
            overhead_warns = overhead_warns + 1;
        end
        if bp_motion_db < -1.0 && bp_hf_db < -1.0 && bp_mains_db < -1.0
            overhead_warns = overhead_warns + 1;
        end
        if overhead_warns == 0
            overhead_status = 'PASS - suppression/reporting metrics are internally consistent';
        elseif overhead_warns <= 2
            overhead_status = 'WARN - inspect plots and morphology before reporting';
        else
            overhead_status = 'FAIL - overhead metrics indicate likely distortion or invalid data';
        end
        ...
        lines_out = {
            ...
            '  CORE FILTER METRICS'
            sep2
            sprintf('  WBSNR improvement : %+.2f dB   (raw %+.2f -> output %+.2f dB)   %s', ...
                    wbsnr_impr, wbsnr_raw, wbsnr_filt, ...
            ...
            '  SUPPRESSION TARGETS'
            sep2
            sprintf('  50 Hz reduction : %+.2f dB   %s', notch_effect, ...
                    ternary_str(notch_effect > 10, '✓ significant', ...
                    ternary_str(notch_effect > 3,  '○ moderate', '○ minimal (recording may be clean)')))
            sprintf('  Wide band       : %+.2f dB   (0.5-%.0f Hz raw -> final)', bp_wide_db, wide_hi_hz)
            sprintf('  Motion band     : %+.2f dB   (0.5-40 Hz raw -> final)', bp_motion_db)
            ''
            ...
            sprintf('  Output health   : %s', overhead_status)
            sprintf('  R peaks / QRS   : %d -> %d peaks | QRS ratio %.2f', ...
                    preserve.n_in, preserve.n_out, preserve.qrs_amp_ratio)
            sprintf('  Total latency   : %.2f us/sample   (budget %.2f us/sample)', ...
                    li.t_total_us, 1e6/Fs)
        };
        ...
        qlines{end+1,1} = '  CORE QUANTITATIVE CHECKS';
        ...
        if n_beats >= 3
            ...
            qlines{end+1,1} = '';
            qlines{end+1,1} = '  BEAT MORPHOLOGY / RINGING';
    ...
    function [out, ref_d, ref_lbl] = apply_mas(ecg_filtered, mas_idx, Fs)
        ...
        FW_NLMS_MU = 0.50;
        FW_NLMS_EPS = 1e-6;
        FW_NLMS_STEP_CAP = 0.001;
        FW_NLMS_LEAK = 0.999995;

        switch mas_idx
            case {32, 33, 35, 36, 37, 38}  % UI: LMS basic reference variants
                [out, ref_d, ref_lbl] = apply_basic_mas(ecg_filtered, mas_idx, Fs, EPS);

            case {47, 48, 49, 50, 51, 52}  % UI: SS-LMS basic reference variants
                [out, ref_d, ref_lbl] = apply_basic_mas(ecg_filtered, mas_idx, Fs, EPS);

            case {53, 54, 55, 56, 57, 58}  % UI: VS-NLMS basic reference variants
                [out, ref_d, ref_lbl] = apply_basic_mas(ecg_filtered, mas_idx, Fs, EPS);

            % ── NLMS scalar |a| ─────────────────────────────────────────
            case 1   % M1: NLMS |a| IMU0
                ref   = rt_condition_mas_reference(imu.mag0_ac, Fs);
                out   = mas_nlms(ecg_filtered, ref, FW_NLMS_MU, FW_NLMS_EPS, 16, FW_NLMS_STEP_CAP, FW_NLMS_LEAK);
                ref_d = ref; ref_lbl = '|a| IMU0 (transport-band causal z)';
            ...
            case 3   % UI: NLMS |a| with selectable IMU source
                [out, ref_d, ref_lbl] = apply_basic_mas(ecg_filtered, mas_idx, Fs, EPS);
            ...
            case {6, 34, 43, 44, 45, 46}  % UI: RLS basic reference variants
                [out, ref_d, ref_lbl] = apply_basic_mas(ecg_filtered, mas_idx, Fs, EPS);
            ...
            case 9   % M9: NLMS 3-axis 3-site (LA+RA+LL)
                ...
                out   = mas_nlms(ecg_filtered, ref, FW_NLMS_MU, FW_NLMS_EPS, 16, FW_NLMS_STEP_CAP, FW_NLMS_LEAK);
                ref_d = ref(:,1); ref_lbl = 'ax0 (transport-band causal z) [3-axis 3-site]';
            ...
            % ── NLMS scalar |g| ─────────────────────────────────────────
            case {13, 39, 40, 41, 42}  % UI: NLMS basic reference variants
                [out, ref_d, ref_lbl] = apply_basic_mas(ecg_filtered, mas_idx, Fs, EPS);
            ...
            case 18  % M18: NLMS 6-axis 3-site (LA+RA+LL)
                ...
                out   = mas_nlms(ecg_filtered, ref, FW_NLMS_MU, FW_NLMS_EPS, 16, FW_NLMS_STEP_CAP, FW_NLMS_LEAK);
                ref_d = ref(:,1); ref_lbl = 'ax0 (transport-band causal z) [6-axis 3-site]';
            ...
            case 21  % M21: Blanked Leaky NLMS |a| 3-site (LA+RA+LL)
                ...
                out   = mas_blanked_leaky_nlms(ecg_filtered, ref, 0.05, 0.02, EPS, Fs);
                ref_d = ref(:,1); ref_lbl = '|a| IMU0 (transport-band causal z) [blanked+leaky 3-site]';
            ...
            case 24  % Legacy feature bank + coherent-band pruning
                [ref_scope, ~] = selected_mas_reference_scope(false);
                [ref_bank, ref_names] = build_rt_reference_bank(imu, Fs, ref_scope);
            ...
            case 26  % Legacy relaxed coherent-band pruning
                [ref_scope, ~] = selected_mas_reference_scope(true);
                [ref_bank, ref_names] = build_rt_reference_bank(imu, Fs, ref_scope);
                [out, ref_d, ref_lbl] = mas_rt_coherence_band_nlms( ...
            ...
            case 31  % Legacy validated rolling adaptive event-band MAS
                [ref_scope, ~] = selected_mas_reference_scope(false);
                [ref_bank, ref_names] = build_rt_reference_bank(imu, Fs, ref_scope);
    ...
    function [out, ref_d, ref_lbl] = apply_basic_mas(ecg_filtered, mas_idx, Fs, eps_reg)
        params = default_mas_params();
        [alg_kind, ref_kind, ~, alg_label] = basic_mas_config(mas_idx);
        base_order = params.tap_order;
        [ref, ref_names, ref_d, ref_lbl] = selected_mas_refs(state.imu, Fs, ref_kind);
        if isempty(ref)
            out = ecg_filtered;
            return;
        end
        ...
        n_ref = size(ref, 2);
        tap_order = basic_mas_tap_order(alg_kind, n_ref, base_order);
        switch alg_kind
            case 'lms'
                mu_lms = basic_lms_mu(n_ref, tap_order, params.lms_mu_cap);
                out = mas_lms(ecg_filtered, ref, mu_lms, tap_order);
                coeff_lbl = sprintf('mu=%.4g', mu_lms);
            case 'sslms'
                mu_lms = basic_lms_mu(n_ref, tap_order, params.lms_mu_cap);
                out = mas_sslms(ecg_filtered, ref, mu_lms, tap_order);
                coeff_lbl = sprintf('mu=%.4g', mu_lms);
            case 'rls'
                out = mas_rls(ecg_filtered, ref, params.rls_lambda, eps_reg, tap_order);
                coeff_lbl = sprintf('lambda=%.5g', params.rls_lambda);
            case 'vs_nlms'
                mu_nlms = basic_nlms_mu(params.nlms_mu_base);
                out = mas_vs_nlms(ecg_filtered, ref, 0.995, 0.005, ...
                    params.nlms_step_cap, params.nlms_eps, tap_order);
                coeff_lbl = sprintf('alpha=0.995, gamma=0.005, cap=%.4g', params.nlms_step_cap);
            otherwise
                mu_nlms = basic_nlms_mu(params.nlms_mu_base);
                out = mas_nlms(ecg_filtered, ref, mu_nlms, params.nlms_eps, ...
                    tap_order, params.nlms_step_cap, params.nlms_weight_leak);
                coeff_lbl = sprintf('mu=%.4g, cap=%.4g, leak=%.6g', ...
                    mu_nlms, params.nlms_step_cap, params.nlms_weight_leak);
        end
        ref_lbl = sprintf('%s [%s ungated, %d refs, %d taps, %s: %s]', ...
            ref_lbl, alg_label, n_ref, tap_order, coeff_lbl, summarize_mas_ref_names(ref_names));
    end

    function params = default_mas_params()
        params.lms_mu_cap = read_numeric_control(mas_lms_cap_edit, 0.01, 1e-7, 0.1);
        params.nlms_mu_base = read_numeric_control(mas_nlms_mu_edit, 0.50, 1e-5, 2.0);
        params.nlms_step_cap = read_numeric_control(mas_nlms_cap_edit, 0.001, 1e-5, 0.5);
        params.nlms_weight_leak = 0.999995;
        params.nlms_eps = 1e-6;
        params.rls_lambda = read_numeric_control(mas_rls_lambda_edit, 0.999, 0.900, 0.99999);
        params.tap_order = round(read_numeric_control(mas_taps_edit, 32, 1, 64));
        bands = zeros(3, 2);
        for bb = 1:3
            band_lo = read_numeric_control(mas_band_lo_edit{bb}, mas_band_defaults(bb, 1), 0.05, 40.0);
            band_hi = read_numeric_control(mas_band_hi_edit{bb}, mas_band_defaults(bb, 2), 0.30, 40.0);
            if band_hi <= band_lo + 0.25
                band_hi = min(40.0, band_lo + 0.25);
            end
            bands(bb, :) = [band_lo, band_hi];
        end
        params.transport_band = bands;
    end

    function v = read_numeric_control(ctrl, fallback, lo, hi)
        v = fallback;
        try
            v = double(ctrl.Value);
        catch
    ...
    function [alg_kind, ref_kind, base_order, alg_label] = basic_mas_config(mas_idx)
        base_order = 16;
        switch mas_idx
            case {32, 33, 35, 36, 37, 38}
                alg_kind = 'lms';
                alg_label = 'LMS';
            case {47, 48, 49, 50, 51, 52}
                alg_kind = 'sslms';
                alg_label = 'SS-LMS';
            case {6, 34, 43, 44, 45, 46}
                alg_kind = 'rls';
                alg_label = 'RLS';
            case {53, 54, 55, 56, 57, 58}
                alg_kind = 'vs_nlms';
                alg_label = 'VS-NLMS';
            otherwise
                alg_kind = 'nlms';
        ...
        ref_kind = mas_ref_kind_from_id(mas_idx);
    end

    function tap_order = basic_mas_tap_order(alg_kind, n_ref, base_order)
        tap_order = base_order;
        if strcmp(alg_kind, 'rls')
            % RLS scales with the square of refs*taps. Keep high-dimensional
            % six-axis source tests responsive while preserving causal updates.
            max_rls_state = 160;
            tap_order = min(base_order, max(4, floor(max_rls_state / max(n_ref, 1))));
        end
    end

    function mu = basic_lms_mu(n_ref, tap_order, mu_cap)
        % Plain LMS is sensitive to input dimension because the update is not
        % power-normalized. Scale the step for large signed-axis banks.
        if nargin < 3 || ~isfinite(mu_cap) || mu_cap <= 0
            mu_cap = 0.01;
        end
        state_dim = max(1, n_ref * tap_order);
        mu = min(mu_cap, 0.05 / state_dim);
    end

    function mu = basic_nlms_mu(mu_base)
        % Firmware-aligned aggressive NLMS. Multi-reference scaling is handled
        % by the instantaneous tapped-delay norm and the per-sample step cap.
        if nargin < 1 || ~isfinite(mu_base) || mu_base <= 0
            mu_base = 0.20;
        end
        mu = mu_base;
    end

    function ref_kind = mas_ref_kind_from_id(mas_idx)
        switch mas_idx
            case {32, 3, 6, 47, 53}
                ref_kind = 'amag';
            case {33, 13, 34, 48, 54}
    ...
    function txt = summarize_mas_ref_names(ref_names)
        if isempty(ref_names)
            txt = 'none';
            return;
        end
        n_show = min(8, numel(ref_names));
        txt = strjoin(ref_names(1:n_show), ', ');
        if numel(ref_names) > n_show
            txt = sprintf('%s, ... (%d total)', txt, numel(ref_names));
        end
    end

    function [scope, pair_label] = selected_mas_reference_scope(varargin)
        [source_key, pair_label] = selected_mas_source();
        switch source_key
            case 'imu0_ll'
                scope = 'site0';
            case 'imu1_la'
            ...
            case 'ra_la'
                scope = 'lead_i';
            case 'lead_matched_ra_pair'
                if selected_ecg_lead_is_ch2()
                    scope = 'lead_ii';
                    pair_label = 'Lead II RA+LL (IMU2+IMU0)';
                else
                    scope = 'lead_i';
                    pair_label = 'Lead I RA+LA (IMU2+IMU1)';
                end
            otherwise
                scope = 'ra_la_ra_ll';
        end
    ...
    function tf = selected_ecg_lead_is_ch2()
        tf = strcmp(ch_dd.Value, 'Lead II (CH2)');
    end

    function [source_key, source_label] = selected_mas_source()
        source_label = mas_source_dd.Value;
        switch source_label
            case 'IMU0 - LL'
                source_key = 'imu0_ll';
                source_label = 'IMU0 / LL';
            case 'IMU1 - LA'
                source_key = 'imu1_la';
                source_label = 'IMU1 / LA';
            case 'IMU2 - RA'
                source_key = 'imu2_ra';
                source_label = 'IMU2 / RA';
            case 'RA+LL'
                source_key = 'ra_ll';
                source_label = 'RA+LL (IMU2+IMU0)';
            case 'RA+LA'
                source_key = 'ra_la';
                source_label = 'RA+LA (IMU2+IMU1)';
            case 'Lead-matched RA-pair'
                source_key = 'lead_matched_ra_pair';
                source_label = 'Lead-matched RA-pair (CH1 RA+LA, CH2 RA+LL)';
            otherwise
                source_key = 'ra_la_ra_ll';
                source_label = 'RA+LA and RA+LL (IMU2+IMU1, IMU2+IMU0)';
        end
    end

    function [ref, ref_names, ref_d, ref_lbl] = selected_scalar_mas_refs(imu, Fs, kind)
        if strcmp(kind, 'g')
            ref_kind = 'gmag';
        else
            ref_kind = 'amag';
        end
        [ref, ref_names, ref_d, ref_lbl] = selected_mas_refs(imu, Fs, ref_kind);
    end

    function [ref, ref_names, ref_d, ref_lbl] = selected_mas_refs(imu, Fs, ref_kind)
        [source_key, source_label] = selected_mas_source();
        [ll, names_ll, tag, unit_lbl] = mas_site_ref(imu, 0, ref_kind);
        [la, names_la, ~, ~] = mas_site_ref(imu, 1, ref_kind);
        [ra, names_ra, ~, ~] = mas_site_ref(imu, 2, ref_kind);

        switch source_key
            case 'imu0_ll'
                raw_ref = ll;
                ref_names = prefix_mas_names(names_ll, 'IMU0/LL_');
            case 'imu1_la'
                raw_ref = la;
                ref_names = prefix_mas_names(names_la, 'IMU1/LA_');
            case 'imu2_ra'
                raw_ref = ra;
                ref_names = prefix_mas_names(names_ra, 'IMU2/RA_');
            case 'ra_ll'
                n = min([size(ll, 1), size(ra, 1)]);
                if n <= 0 || size(ll, 2) ~= size(ra, 2)
                    raw_ref = [];
                    ref_names = {};
                else
                    ll = ll(1:n, :);
                    ...
                    raw_ref = [ra, ll, ra - ll];
                    ref_names = [ ...
                        prefix_mas_names(names_ra, 'RA_'), ...
                        prefix_mas_names(names_ll, 'LL_'), ...
                        prefix_mas_names(names_ra, 'RA_LL_')];
                end
            case 'ra_la'
                n = min([size(la, 1), size(ra, 1)]);
                if n <= 0 || size(la, 2) ~= size(ra, 2)
                    raw_ref = [];
                    ref_names = {};
                else
                    la = la(1:n, :);
                    ...
                    ref_names = [ ...
                        prefix_mas_names(names_ra, 'RA_'), ...
                        prefix_mas_names(names_la, 'LA_'), ...
                        prefix_mas_names(names_ra, 'RA_LA_')];
                end
            case 'lead_matched_ra_pair'
                if selected_ecg_lead_is_ch2()
                    source_label = 'Lead II RA+LL (IMU2+IMU0)';
                    n = min([size(ll, 1), size(ra, 1)]);
                    if n <= 0 || size(ll, 2) ~= size(ra, 2)
                        raw_ref = [];
                        ref_names = {};
                    ...
                    source_label = 'Lead I RA+LA (IMU2+IMU1)';
                    n = min([size(la, 1), size(ra, 1)]);
                    if n <= 0 || size(la, 2) ~= size(ra, 2)
                        raw_ref = [];
                        ref_names = {};
            ...
            otherwise
                n = min([size(ll, 1), size(la, 1), size(ra, 1)]);
                if n <= 0 || size(ll, 2) ~= size(la, 2) || size(ll, 2) ~= size(ra, 2)
                    raw_ref = [];
                    ref_names = {};
                else
                    ll = ll(1:n, :);
        ...
        params = default_mas_params();
        transport_band = rt_sanitize_transport_bands(Fs, params.transport_band);
        ref = rt_condition_mas_reference(raw_ref, Fs, transport_band);
        ref_names = expand_mas_ref_names_for_bands(ref_names, transport_band);
        if isempty(ref)
            ref_d = [];
            ref_lbl = sprintf('%s %s bypass: no usable %s reference', tag, source_label, unit_lbl);
        else
            ref_d = ref(:, 1);
            ref_lbl = sprintf('%s %s (%s, %s transport refs)', ...
                tag, source_label, unit_lbl, format_transport_bands(transport_band));
        end
    end

    function [raw, names, tag, unit_lbl] = mas_site_ref(imu, site_idx, ref_kind)
        [accel_xyz, gyro_xyz, amag, gmag] = mas_site_arrays(imu, site_idx);
        switch ref_kind
            case 'gmag'
                raw = gmag;
                names = {'gmag'};
                tag = '|g|';
                unit_lbl = 'gyro magnitude';
            case 'magpair'
                raw = mas_align_columns({amag, gmag});
                names = {'amag', 'gmag'};
                tag = '|a|+|g|';
                unit_lbl = 'accel and gyro magnitudes';
            case 'accel3'
                raw = accel_xyz;
                names = {'ax', 'ay', 'az'};
                tag = 'accel xyz';
                unit_lbl = 'signed accelerometer axes';
            case 'gyro3'
                raw = gyro_xyz;
                names = {'gx', 'gy', 'gz'};
                tag = 'gyro xyz';
                unit_lbl = 'signed gyroscope axes';
            case 'six'
                raw = mas_align_columns({accel_xyz, gyro_xyz});
                names = {'ax', 'ay', 'az', 'gx', 'gy', 'gz'};
                tag = 'accel+gyro 6-axis';
                unit_lbl = 'signed accel and gyro axes';
            otherwise
                raw = amag;
                names = {'amag'};
                tag = '|a|';
                unit_lbl = 'accel magnitude; gravity removed';
        end
    end

    function [accel_xyz, gyro_xyz, amag, gmag] = mas_site_arrays(imu, site_idx)
        suffix = sprintf('%d_ac', site_idx);
        accel_xyz = mas_align_columns({ ...
            mas_imu_col(imu, ['ax' suffix]), ...
            mas_imu_col(imu, ['ay' suffix]), ...
            mas_imu_col(imu, ['az' suffix])});
        gyro_xyz = mas_align_columns({ ...
            mas_imu_col(imu, ['gx' suffix]), ...
            mas_imu_col(imu, ['gy' suffix]), ...
            mas_imu_col(imu, ['gz' suffix])});
        amag = mas_imu_col(imu, ['mag' suffix]);
        gmag = mas_imu_col(imu, ['gmag' suffix]);
    end

    function col = mas_imu_col(imu, field_name)
        if isfield(imu, field_name)
            col = double(imu.(field_name)(:));
        else
            col = [];
        end
    end

    function mat = mas_align_columns(cols)
        n_blocks = numel(cols);
        row_counts = zeros(1, n_blocks);
        col_counts = zeros(1, n_blocks);
        for kk = 1:n_blocks
            block = double(cols{kk});
            if isvector(block)
                block = block(:);
            end
            cols{kk} = block;
            row_counts(kk) = size(block, 1);
            col_counts(kk) = size(block, 2);
        end
        n = min(row_counts);
        if isempty(n) || n <= 0
            mat = [];
        ...
        mat = zeros(n, sum(col_counts));
        jj = 1;
        for kk = 1:n_blocks
            width = col_counts(kk);
            mat(:, jj:(jj + width - 1)) = cols{kk}(1:n, :);
            jj = jj + width;
        end
    end

    function names_out = prefix_mas_names(names_in, prefix)
        if isempty(names_in)
            names_out = {};
        else
            names_out = cellfun(@(s) [prefix s], names_in, 'UniformOutput', false);
        end
    end

    function ref = selected_mas_metric_ref(mas_idx, N_req, Fs)
        ref = [];
        if N_req <= 0 || isempty(state.imu.mag0_ac)
            return;
        end
        ref_kind = mas_ref_kind_from_id(mas_idx);
        [ref_mat, ~, ~, ~] = selected_mas_refs(state.imu, Fs, ref_kind);
        if ~isempty(ref_mat) && size(ref_mat, 1) >= N_req
            ref = ref_mat(1:N_req, 1);
        end
    end

    function refs = selected_mas_metric_refs(mas_idx, N_req, Fs)
        refs = [];
        if N_req <= 0 || isempty(state.imu.mag0_ac)
            return;
        end
        ...
        [ref_mat, ~, ~, ~] = selected_mas_refs(state.imu, Fs, ref_kind);
        if ~isempty(ref_mat)
            n = min(N_req, size(ref_mat, 1));
            refs = ref_mat(1:n, :);
        end
    end

    function [ref_bank, ref_names] = build_auto_mas_refs(imu)
        % Source-controlled reference bank for the current MAS path.
        [scope, ~] = selected_mas_reference_scope(false);
        [ref_bank, ref_names] = build_rt_reference_bank(imu, state.fs, scope);
    ...
    function update_mas_tab(sig_in, filtered, after_mas, ref_display, ref_disp_lbl, ...
        ...
        mlines{end+1,1} = sprintf('  MAS:       %s', display_mas_name(mas_idx));
        mlines{end+1,1} = sprintf('  PIPELINE:  %s', combo_label);
        ...
        mlines{end+1,1} = '  IMU PREPROC:   accel/gyro axes use causal gravity/DC removal.';
        mlines{end+1,1} = '  NOTCH:     N1 fixed 50 Hz; N2/N3/N4/N5 adapt 50 Hz amplitude/phase only.';
        mlines{end+1,1} = '  ADAPTATION:    LMS/NLMS/RLS adapt continuously.';
        mlines{end+1,1} = sep_m;
        mlines{end+1,1} = '';
        ...
        % ── Band indices ───────────────────────────────────────────────
        full_hi_hz = max(f_pw);
        full_band = f_pw >= 0.5 & f_pw <= full_hi_hz;
        ma_band  = f_pw >= 0.5 & f_pw <= min(40.0, 0.45*Fs);
        qrs_band = f_pw >= 8   & f_pw <= 35;

        % ① ISOLATED MAS POWER EFFECT (input vs direct output of MAS only)
        E_in_full  = sum(P_mas_in(full_band))  + 1e-30;
        E_out_full = sum(P_mas_out(full_band)) + 1e-30;
        ...
        full_db = 10*log10(E_in_full / E_out_full); % positive = ECG-band power reduced
        ...
        if ~isempty(ref_display) && numel(ref_display) >= N_coh
            imu_metric_refs = ref_display(1:N_coh);
        else
            imu_metric_refs = selected_mas_metric_refs(mas_idx, N_coh, Fs);
        end
        imu_ref = mas_reference_envelope(imu_metric_refs, Fs);
        if ~isempty(imu_ref)
            ...
            n_env = min(N_coh, numel(imu_ref));
            qm = rt_causal_qrs_mask(mas_input(1:n_env), Fs);
            ecg_b_m  = mas_input(1:n_env);       ecg_b_m  = ecg_b_m(qm);
            ecg_a_m  = mas_direct_out(1:n_env);  ecg_a_m  = ecg_a_m(qm);
            imu_m    = imu_ref(1:n_env);         imu_m    = imu_m(qm);
            n_coh_v  = sum(qm);
            ...
            if n_coh_v >= 2 * coh_win
                ...
                cb = f_coh >= 0.5 & f_coh <= min(40.0, 0.45*Fs);
        ...
        % ④ LOW-MOTION MAS DELTA DIAGNOSTIC
        % Compare MAS input vs direct output only during low-IMU windows.
        low_diag = struct('median_pct', NaN, 'p95_pct', NaN, 'n_windows', 0);
        N_lm = min(numel(mas_input), numel(mas_direct_out));
        if ~isempty(ref_display) && numel(ref_display) >= N_lm
            imu_metric_refs_lm = ref_display(1:N_lm);
        else
            imu_metric_refs_lm = selected_mas_metric_refs(mas_idx, N_lm, Fs);
        end
        imu_env = mas_reference_envelope(imu_metric_refs_lm, Fs);
        if ~isempty(imu_env)
            low_diag = low_motion_delta_metrics(mas_input(1:N_lm), mas_direct_out(1:N_lm), imu_env, Fs);
        end

        % ── Format output ─────────────────────────────────────────────
        mlines{end+1,1} = '  CORE MAS METRICS';
        mlines{end+1,1} = sprintf('  Comparing: %s -> %s', in_lbl, out_lbl);
        mlines{end+1,1} = '  Metrics isolate MAS only, not BPF/notch effects.';
        mlines{end+1,1} = sep_m;
        mlines{end+1,1} = '';

        mlines{end+1,1} = '  SPECTRAL CHANGE / ECG PRESERVATION';
        mlines{end+1,1} = repmat('·',1,65);
        mlines{end+1,1} = sprintf('  Full-band power drop   : %+.2f dB   (0.5-%.0f Hz)', full_db, full_hi_hz);
        mlines{end+1,1} = sprintf('  0.5-40 Hz power drop   : %+.2f dB   %s', mbpr_db, ...
                           ternary_str(mbpr_db > 2,  'stronger residual reduction', ...
                           ternary_str(mbpr_db > 0,  'mild residual reduction', 'residual power increased')));
        ...
        mlines{end+1,1} = sprintf('  R peaks               : %d -> %d   ratio %.2f   %s', ...
                           numel(r_in), numel(r_out), rp_ratio, ...
                           peak_count_label(rp_ratio));
        mlines{end+1,1} = '';

        mlines{end+1,1} = '  IMU COUPLING / LOW-MOTION DISTORTION';
        mlines{end+1,1} = repmat('·',1,65);
        if isnan(coh_before)
            mlines{end+1,1} = '  IMU coupling          : unavailable';
        else
            mlines{end+1,1} = sprintf('  IMU-ECG gamma^2       : %.3f -> %.3f   Δ%+.3f   %s', ...
                               coh_before, coh_after, dcoh, ...
                               ternary_str(dcoh < -0.05, '✓ coupling broken', ...
                               ternary_str(dcoh <  0.01, '○ marginal',        '✗ unchanged')));
        end
        if isnan(low_diag.median_pct)
            mlines{end+1,1} = '  Low-motion MAS delta  : unavailable';
        else
            mlines{end+1,1} = sprintf('  Low-motion MAS delta  : median %.1f%%, p95 %.1f%% of ECG scale (%d windows)   %s', ...
                               low_diag.median_pct, low_diag.p95_pct, low_diag.n_windows, ...
                               low_motion_delta_label(low_diag.median_pct, low_diag.p95_pct));
        end
        mlines{end+1,1} = sep_m;
    ...
    function notes = get_mas_notes(mas_idx)
        switch mas_idx
            ...
            case 3,  notes = '  MAS |a| - NLMS using the selected accelerometer magnitude reference. Accel axes use causal gravity/DC removal before |a| is formed.';
            ...
            case 6,  notes = '  RLS |a| - recursive least-squares using the selected accelerometer magnitude reference.';
            case 34, notes = '  RLS |g| - recursive least-squares using the selected gyroscope magnitude reference.';
            ...
            case 13, notes = '  MAS |g| - NLMS using the selected gyroscope magnitude reference.';
            ...
            case 24, notes = '  M24 RT source coherent-band NLMS - selects only source feature/lag/band candidates with strong ECG-IMU coherence, then subtracts those narrowband artefact estimates.';
            case 25, notes = '  M25 RT staged LA/RA NLMS - two-stage active-electrode correction using IMU1=LA then IMU2=RA feature+lag refs.';
            case 26, notes = '  M26 RT source relaxed coherent-band - same coherent-band NLMS idea as M24, but with a slightly lower coherence threshold.';
            ...
            case 31, notes = '  M31 Validated adaptive source event-band - rolling train/validation MAS; subtracts only when coherence, envelope correlation, burst overlap, and held-out reduction pass.';
            case 32, notes = '  LMS |a| - stochastic-gradient ANC using the selected accelerometer magnitude reference.';
            case 33, notes = '  LMS |g| - stochastic-gradient ANC using the selected gyroscope magnitude reference.';
            case 35, notes = '  LMS |a|+|g| - paired magnitude test; checks whether accel+gyro magnitude helps beyond either alone.';
            case 36, notes = '  LMS accel xyz - signed accelerometer-axis reference bank.';
            case 37, notes = '  LMS gyro xyz - signed gyroscope-axis reference bank; useful when electrode motion is mainly rotational.';
            case 38, notes = '  LMS accel+gyro 6-axis - largest LMS basic bank. Raw-ECG order can still be a stress test; prefer BPF+Notch before MAS.';
            case 39, notes = '  NLMS |a|+|g| - normalized update using paired accel and gyro magnitudes.';
            case 40, notes = '  NLMS accel xyz - normalized signed accelerometer-axis reference bank.';
            case 41, notes = '  NLMS gyro xyz - normalized signed gyroscope-axis reference bank.';
            case 42, notes = '  NLMS accel+gyro 6-axis - strongest basic NLMS bank; use to test whether richer causal IMU refs help.';
            case 43, notes = '  RLS |a|+|g| - fast-converging least-squares update with paired accel and gyro magnitudes.';
            case 44, notes = '  RLS accel xyz - signed accelerometer-axis RLS. High-dimensional sources auto-use fewer taps to keep the GUI responsive.';
            case 45, notes = '  RLS gyro xyz - signed gyroscope-axis RLS. High-dimensional sources auto-use fewer taps to keep the GUI responsive.';
            case 46, notes = '  RLS accel+gyro 6-axis - largest basic RLS test. Causal but compute-heavy and more prone to overfitting.';
            otherwise, notes = '';
        end
    end

    function out = overhead_preservation_metrics(sig_in, sig_out, Fs)
        sig_in = double(sig_in(:));
        sig_out = double(sig_out(:));
        n = min(numel(sig_in), numel(sig_out));
        sig_in = sig_in(1:n);
        sig_out = sig_out(1:n);

        r_in = detect_rpeaks(sig_in, Fs);
        r_out = detect_rpeaks(sig_out, Fs);
        out.n_in = numel(r_in);
        out.n_out = numel(r_out);
        out.ratio = out.n_out / max(out.n_in, 1);
        out.qrs_amp_ratio = NaN;
        out.t_amp_ratio = NaN;
        out.t_prd = NaN;
        out.st_prd = NaN;

        if numel(r_in) < 3 || n < round(0.7*Fs)
            return;
        end

        pre_base = round(0.20 * Fs);
        base_end = round(0.06 * Fs);
        qrs_pre = round(0.05 * Fs);
        qrs_post = round(0.10 * Fs);
        st_start = round(0.06 * Fs);
        st_end = round(0.12 * Fs);
        t_start = round(0.12 * Fs);
        t_end = round(0.42 * Fs);

        max_beats = numel(r_in);
        qrs_ratios = NaN(max_beats, 1);
        t_ratios = NaN(max_beats, 1);
        t_prds = NaN(max_beats, 1);
        st_prds = NaN(max_beats, 1);
        qrs_count = 0;
        t_count = 0;
        st_count = 0;
        ...
        for kk = 1:numel(r_in)
            pk = r_in(kk);
            if pk - pre_base < 1 || pk + t_end > n
                continue;
            end
            base_idx = (pk-pre_base):(pk-base_end);
            qrs_idx = (pk-qrs_pre):(pk+qrs_post);
            st_idx = (pk+st_start):(pk+st_end);
            t_idx = (pk+t_start):(pk+t_end);

            bin = median(sig_in(base_idx), 'omitnan');
            bout = median(sig_out(base_idx), 'omitnan');
            xi_qrs = sig_in(qrs_idx) - bin;
            xo_qrs = sig_out(qrs_idx) - bout;
            xi_st = sig_in(st_idx) - bin;
            xo_st = sig_out(st_idx) - bout;
            xi_t = sig_in(t_idx) - bin;
            xo_t = sig_out(t_idx) - bout;

            qrs_amp = max(xi_qrs) - min(xi_qrs);
            qrs_amp_out = max(xo_qrs) - min(xo_qrs);
            t_amp = max(abs(xi_t));
            t_amp_out = max(abs(xo_t));

            if qrs_amp > 1e-9
                qrs_count = qrs_count + 1;
                qrs_ratios(qrs_count) = qrs_amp_out / qrs_amp;
            end
            if t_amp > 1e-9
                t_count = t_count + 1;
                t_ratios(t_count) = t_amp_out / t_amp;
                t_prds(t_count) = 100 * sqrt(sum((xi_t - xo_t).^2) / (sum(xi_t.^2) + 1e-12));
            end
            if sum(xi_st.^2) > 1e-12
                st_count = st_count + 1;
                st_prds(st_count) = 100 * sqrt(sum((xi_st - xo_st).^2) / (sum(xi_st.^2) + 1e-12));
            end
        end

        if qrs_count > 0, out.qrs_amp_ratio = median(qrs_ratios(1:qrs_count), 'omitnan'); end
        if t_count > 0
            out.t_amp_ratio = median(t_ratios(1:t_count), 'omitnan');
            out.t_prd = median(t_prds(1:t_count), 'omitnan');
        end
        if st_count > 0, out.st_prd = median(st_prds(1:st_count), 'omitnan'); end
    end

    function rr = robust_range(x, lo_pct, hi_pct)
        x = double(x(:));
        x = x(isfinite(x));
        if isempty(x)
            rr = NaN;
        ...
        rr = prctile(x, hi_pct) - prctile(x, lo_pct);
    end

    function db = band_reduction_db(Pin, Pout, f, lo, hi)
        band = f >= lo & f <= hi;
        if ~any(band)
            db = NaN;
            return;
        end
        db = 10 * log10((sum(Pin(band)) + 1e-30) / (sum(Pout(band)) + 1e-30));
    end

    function env = mas_reference_envelope(refs, Fs)
        env = [];
        if isempty(refs)
            return;
        end
        refs = double(refs);
        if isvector(refs), refs = refs(:); end
        good = all(isfinite(refs), 1) & std(refs, 0, 1) > 1e-10;
        refs = refs(:, good);
        if isempty(refs)
            return;
        end
        refs = refs - median(refs, 1, 'omitnan');
        scale = median(abs(refs), 1, 'omitnan') + eps;
        refs = refs ./ scale;
        env = sqrt(mean(refs.^2, 2, 'omitnan'));
        if isfinite(Fs) && Fs > 0 && numel(env) >= 3
            win = max(3, round(0.25 * Fs));
            env = movmean(env, win, 'Endpoints', 'shrink');
        end
    end

    function diag = low_motion_delta_metrics(sig_in_lm, sig_out_lm, imu_env, Fs)
        diag = struct('median_pct', NaN, 'p95_pct', NaN, 'n_windows', 0);
        sig_in_lm = double(sig_in_lm(:));
        sig_out_lm = double(sig_out_lm(:));
        imu_env = double(imu_env(:));
        Nlm = min([numel(sig_in_lm), numel(sig_out_lm), numel(imu_env)]);
        if Nlm < max(32, round(2 * Fs))
            return;
        end
        sig_in_lm = sig_in_lm(1:Nlm);
        sig_out_lm = sig_out_lm(1:Nlm);
        imu_env = imu_env(1:Nlm);
        win = max(32, round(2.0 * Fs));
        hop = max(1, round(1.0 * Fs));
        starts = 1:hop:(Nlm - win + 1);
        if isempty(starts)
            return;
        end
        motion_level = NaN(numel(starts), 1);
        delta_pct = NaN(numel(starts), 1);
        scale = robust_range(sig_in_lm, 5, 95);
        if ~isfinite(scale) || scale < 1e-9
            scale = 6 * rms(sig_in_lm(isfinite(sig_in_lm)));
        end
        if ~isfinite(scale) || scale < 1e-9
            return;
        end
        for kk = 1:numel(starts)
            idx = starts(kk):(starts(kk) + win - 1);
            motion_level(kk) = median(imu_env(idx), 'omitnan');
            xi = sig_in_lm(idx);
            xo = sig_out_lm(idx);
            good = isfinite(xi) & isfinite(xo);
            if nnz(good) < max(16, round(0.5 * Fs))
                continue;
            end
            delta = xo(good) - xi(good);
            delta = delta - median(delta, 'omitnan');
            delta_pct(kk) = 100 * rms(delta) / scale;
        end
        good_windows = isfinite(motion_level) & isfinite(delta_pct);
        if nnz(good_windows) < 2
            return;
        end
        low_thr = prctile(motion_level(good_windows), 40);
        low_vals = delta_pct(good_windows & motion_level <= low_thr);
        if isempty(low_vals)
            return;
        end
        diag.median_pct = median(low_vals, 'omitnan');
        diag.p95_pct = prctile(low_vals, 95);
        diag.n_windows = numel(low_vals);
    end

    function label = low_motion_delta_label(med_pct, p95_pct)
        if med_pct < 5 && p95_pct < 12
            label = 'low distortion';
        elseif med_pct < 12 && p95_pct < 25
            label = 'review plots';
        else
            label = 'possible over-adaptation';
        end
    end

    function label = peak_count_label(ratio)
        if ~isfinite(ratio)
            label = 'unavailable';
        elseif ratio >= 0.90 && ratio <= 1.10
            label = 'stable count';
        elseif ratio >= 0.75 && ratio <= 1.25
            label = 'review detector/plots';
        else
            label = 'peak count changed';
        end
    end
    ...
    function r = detect_rpeaks(sig, fs)
        sig = double(sig(:));
        r = [];
        if numel(sig) < 8 || ~isfinite(fs) || fs <= 0
            return;
        end
        good = isfinite(sig);
        if nnz(good) < 8
        ...
        if any(~good)
            ii_all = (1:numel(sig))';
            sig(~good) = interp1(ii_all(good), sig(good), ii_all(~good), 'linear', 'extrap');
        end
        base_win = odd_window(0.60, fs, numel(sig));
        x = sig - movmedian(sig, base_win, 'Endpoints', 'shrink');
        dx = [0; diff(x)];
        env_win = max(3, round(0.080 * fs));
        score = movmean(dx.^2, env_win, 'Endpoints', 'shrink');
        med_score = median(score, 'omitnan');
        mad_score = median(abs(score - med_score), 'omitnan') + eps;
        thr = max(med_score + 4 * mad_score, prctile(score, 80));
        min_rr = max(1, round(0.32 * fs));
        search = max(1, round(0.08 * fs));
        ii = 2;
        while ii < numel(score)
            if score(ii) >= thr && score(ii) >= score(ii-1) && score(ii) >= score(ii+1)
                lo = max(1, ii - search);
                hi = min(numel(x), ii + search);
                [~, lp] = max(abs(x(lo:hi)));
                pk = lo + lp - 1;
                if isempty(r) || (pk - r(end)) >= min_rr
                    r(end+1) = pk; %#ok<AGROW>
                elseif abs(x(pk)) > abs(x(r(end)))
                    r(end) = pk;
                end
                ii = ii + min_rr;
            else
...
%% ----
    ...
    function y = finite_trace(x, fallback)
        y = real(double(x(:)));
        if isempty(y)
            return;
        end
        bad = ~isfinite(y);
        if any(bad) && nargin >= 2 && ~isempty(fallback)
            fb = real(double(fallback(:)));
            n_fb = min(numel(fb), numel(y));
            if n_fb > 0
                fb = fb(1:n_fb);
                replace = bad(1:n_fb) & isfinite(fb);
                yy = y(1:n_fb);
                yy(replace) = fb(replace);
                y(1:n_fb) = yy;
            end
        end
        good = isfinite(y);
        if nnz(good) >= 2 && any(~good)
            xi = (1:numel(y))';
            y(~good) = interp1(xi(good), y(good), xi(~good), 'linear', 'extrap');
        elseif any(~good)
            y(~good) = 0;
        end
        y(~isfinite(y)) = 0;
    ...
    function [P, f] = safe_pwelch(x, Fs, Nfft)
        ...
        if numel(x) < 4
            ...
            try
                [P, f] = pwelch(finite_trace(x, []), pw_win, pw_nov, Nfft, Fs);
            catch
                X = fft(finite_trace(x, []), Nfft);
                P = abs(X(1:Nfft/2+1)).^2 / max(Fs*numel(x), 1);
                f = (0:Nfft/2)' * Fs / Nfft;
            end
        end
...
function y = mas_sslms(d, x_ref, mu, filter_order)
% Sign-sign LMS: w += mu * sign(e) * sign(x). No multiplications in weight update.
    if nargin < 4, filter_order = 16; end
    d = double(d(:));
    if isvector(x_ref), x_ref = x_ref(:); end
    x_ref = double(x_ref);
    ...
    for n = 1:N
        ...
        x = buf(:);
        e = d(n) - w' * x;
        w = w + mu * sign(e) * sign(x);
        y(n) = e;
    end
end

function y = mas_nlms(d, x_ref, mu, eps_reg, filter_order, step_cap, weight_leak)
...
% step_cap:     optional per-sample normalized update cap, matching firmware
% weight_leak:  optional multiplicative leak applied after each update
...
% Sayed 2003 §9.2; Thakor & Zhu 1991 for ECG MAS application.
    ...
    if nargin < 6 || isempty(step_cap) || ~isfinite(step_cap) || step_cap <= 0
        step_cap = Inf;
    end
    if nargin < 7 || isempty(weight_leak) || ~isfinite(weight_leak) || weight_leak <= 0
        weight_leak = 1.0;
    end
    d = double(d(:));
    N = numel(d);
    ...
    for n = 1:N
        ...
        xTx = x' * x + eps_reg;
        e   = d(n) - w' * x;
        step = min(step_cap, mu / xTx);
        w   = (w + step * e * x) * weight_leak;
        y(n) = e;
    end
end
...
% motion/cable peaks can be high.
    ...
    for rr = 1:size(ref_bank,2)
        ...
        band_all = f >= 0.5 & f <= min(40.0, 0.45*Fs) & isfinite(cxy);
        if ~any(band_low)
            continue;
        end
...
% differential active-electrode motion.
    ...
    switch lower(scope)
        ...
        case 'lead_i'
            ref_bank = [b2, b1, -bd12];
            ref_names = [prefix_ref_names(n2, 'RA_'), prefix_ref_names(n1, 'LA_'), prefix_ref_names(nd12, 'RA_LA_')];
        case 'lead_ii'
            ref_bank = [b2, b0, -bd02];
            ref_names = [prefix_ref_names(n2, 'RA_'), prefix_ref_names(n0, 'LL_'), prefix_ref_names(nd02, 'RA_LL_')];
        case 'lead_i_diff'
            ref_bank = bd12;
            ref_names = prefix_ref_names(nd12, 'LA_RA_');
        case 'lead_ii_diff'
            ref_bank = bd02;
            ref_names = prefix_ref_names(nd02, 'LL_RA_');
        case 'ra_la_ra_ll'
            ref_bank = [b2, b1, b0, -bd12, -bd02];
            ref_names = [ ...
                prefix_ref_names(n2, 'RA_'), prefix_ref_names(n1, 'LA_'), ...
                prefix_ref_names(n0, 'LL_'), prefix_ref_names(nd12, 'RA_LA_'), ...
                prefix_ref_names(nd02, 'RA_LL_')];
        otherwise
...
function names_out = prefix_ref_names(names_in, prefix)
    if isempty(names_in)
        names_out = {};
        return;
    end
...
function y = rt_condition_mas_reference(x, Fs, bands)
% Shape IMU references for transport-band MAS. Keep the path causal: isolate
% each selected vehicle/electrode-motion band, then normalise with a running
% power estimate rather than full-recording mean/std.
    x = double(x);
    if isempty(x)
        y = x;
        return;
    ...
    if nargin < 3 || isempty(bands)
        bands = [0.5 1.5; 10.0 17.0; 28.0 32.0];
    end
    bands = rt_sanitize_transport_bands(Fs, bands);
    band_refs = cell(1, size(bands, 1));
    for bb = 1:size(bands, 1)
        band_refs{bb} = rt_bandpass_if_possible(x, Fs, bands(bb, :));
    end
    y = [band_refs{:}];
    y(~isfinite(y)) = 0;
    y = rt_causal_standardize(y, Fs);
end

function bands = rt_sanitize_transport_bands(Fs, bands)
    ...
    if size(bands, 2) ~= 2 && size(bands, 1) == 2
        bands = bands';
    end
    if size(bands, 2) ~= 2
        bands = [0.5 1.5; 10.0 17.0; 28.0 32.0];
    end
    bands = bands(all(isfinite(bands), 2), :);
    if isempty(bands)
        bands = [0.5 1.5; 10.0 17.0; 28.0 32.0];
    end

    max_hi = min(40.0, 0.45*Fs);
    lo_max = max(0.05, max_hi - 0.25);
    clean = zeros(size(bands));
    n_clean = 0;
    for bb = 1:size(bands, 1)
        lo = min(max(bands(bb, 1), 0.05), lo_max);
        hi = min(max(bands(bb, 2), lo + 0.25), max_hi);
        if hi > lo
            n_clean = n_clean + 1;
            clean(n_clean, :) = [lo, hi];
        end
    ...
    if n_clean == 0
        bands = [0.5, min(1.5, max_hi)];
    else
        bands = sortrows(clean(1:n_clean, :), 1);
    end
end

function band = rt_sanitize_transport_band(Fs, band)
    bands = rt_sanitize_transport_bands(Fs, band);
    band = bands(1, :);
end

function names_out = expand_mas_ref_names_for_bands(ref_names, bands)
    if isempty(ref_names)
        names_out = {};
        return;
    end
    bands = double(bands);
    if isvector(bands), bands = bands(:)'; end
    names_out = cell(1, numel(ref_names) * size(bands, 1));
    kk = 0;
    for bb = 1:size(bands, 1)
        band_tag = sprintf('@%.3g-%.3gHz', bands(bb, 1), bands(bb, 2));
        for rr = 1:numel(ref_names)
            kk = kk + 1;
            names_out{kk} = [ref_names{rr} band_tag];
        end
    end
end

function txt = format_transport_bands(bands)
    bands = double(bands);
    if isvector(bands), bands = bands(:)'; end
    parts = cell(1, size(bands, 1));
    for bb = 1:size(bands, 1)
        parts{bb} = sprintf('%.3g-%.3g', bands(bb, 1), bands(bb, 2));
    end
    txt = [strjoin(parts, ', ') ' Hz'];
end
...
function y = rt_dc_block(x, alpha)
    ...
    for n = 1:size(x, 1)
        y(n, :) = x(n, :) - lp;
        lp = alpha * lp + (1 - alpha) * x(n, :);
    end
end

function y = rt_causal_standardize(x, Fs)
% Causal per-column normalization for adaptive references.
% Uses current/past samples only; first samples intentionally warm up slowly.
    x = double(x);
    if isempty(x)
        y = x;
        return;
    ...
    for n = 1:size(x, 1)
        xn = x(n, :);
        xn(~isfinite(xn)) = mu(~isfinite(xn));
        xc = xn - mu;
        p = alpha * p + (1 - alpha) * (xc .^ 2);
        scale = sqrt(max(p, 1e-6));
        y(n, :) = xc ./ scale;
