...
Phase 1 real-time ECG + IMU monitor and per-condition recorder. Reads the
NXP firmware UART stream, plots ECG / IMU, and writes recordings into
Support_Tools/Recordings/New_ADS1293_IMU_TS_Recordings/ tagged by
selected condition.
"""
...
ECG_ONLY_TARGET_FS_HZ = 500.0
RATE_WARN_FRACTION = 0.15
# New captures stay beside the curated support recording set.
RECORDINGS_DIR = str(_support_recording_dir("New_ADS1293_IMU_TS_Recordings"))
MAX_TERM_ROWS  = 500
...
def expected_fs_for_mode(mode: str) -> Optional[float]:
    if mode.startswith("ADS1293"):
        return ADS1293_TARGET_FS_HZ
    if mode.startswith("ECG_ONLY"):
        return ECG_ONLY_TARGET_FS_HZ
    return None

# ADS1293 display scaling
...
def prepare_ads_display(raw_mV: np.ndarray, raw_codes: np.ndarray) -> tuple[np.ndarray, float, int]:
    ...
    if y.size >= 2:
        dy = np.diff(y)
        jumps = np.where(np.isfinite(dy) &
                         (np.abs(dy) > ADS_DISPLAY_STEP_MV) &
                         ((np.abs(y[:-1]) > ADS_DISPLAY_CLIP_MV) |
                          (np.abs(y[1:]) > ADS_DISPLAY_CLIP_MV)))[0]
        if jumps.size:
            keep[jumps] = False
            keep[jumps + 1] = False
...
class Sample:
    ...
    sel_ch: list[int]   = field(default_factory=lambda: [0, 0])
    sel_mV: list[float] = field(default_factory=lambda: [0.0, 0.0])
    has_sel: bool       = False
    p4_cycles: int      = 0
    m4_cycles: int      = 0
...
class SerialReader(threading.Thread):
    ...
    _HDR_ADS1293_IMU = ("t_us,ads_ch1,ads_ch2,"
                        ...
                        "ax1,ay1,az1,gx1,gy1,gz1,"
                        "ax2,ay2,az2,gx2,gy2,gz2")
    _HDR_ADS1293_IMU_TS = (_HDR_ADS1293_IMU + ",dt0_us,dt1_us,dt2_us")
    _HDR_ADS1293_PHASE4_COMPACT = (
        "t_us,ads_ch1,ads_ch2,p4_ra_pair_ch1,p4_ra_pair_ch2,"
        "p4_ch1,p4_ch2,p4_primary,sel_ch1,sel_ch2,primary_lead,p4_flags,"
        "motion_x10,sqi1,sqi2,hr1_x10,hr2_x10,rmssd1_x10,rmssd2_x10,"
    ...
    def __init__(self, logger: Logger) -> None:
        ...
        self.cm7_hz: Optional[float] = None
        self.cm4_hz: Optional[float] = None
        self._inferred_header_logged = False
    ...
    def _maybe_parse_clocks(self, line: str) -> None:
        # Boot log: "[BOOT] Clocks SystemCore=.. M7=<hz> M4=<hz> ..".
        # Captures the CM7/CM4 core clocks so cycle counts convert to time.
        if "Clocks" not in line:
            return
        for tok in line.split():
            if tok.startswith("M7="):
                try: self.cm7_hz = float(tok[3:])
                except ValueError: pass
            elif tok.startswith("M4="):
                try: self.cm4_hz = float(tok[3:])
                except ValueError: pass
    ...
    def _parse(self, line: str) -> Optional[Sample]:
        parts = line.split(",")
        if len(parts) not in (2, 3, 4, 15, 19, 20, 21, 22, 23, 24):
            return None
        try:
        ...
        if len(vals) == 3:
            ...
            return s
        phase4_header = self.csv_header is not None and "p4_" in self.csv_header
        phase4_heuristic = (
            len(vals) in (21, 22, 23)
            and 0 <= vals[8] <= 8
            and 0 <= vals[9] <= 8
            and 0 <= vals[10] <= 2
            and 0 <= vals[11] <= 4095
        )
        if len(vals) in (15, 19, 20) or (len(vals) in (21, 22, 23) and (phase4_header or phase4_heuristic)):
            s.mode = "ADS1293_PHASE4_COMPACT"
            s.has_ads = True
            s.ecg_corr = vals[1]
            s.ecg_mV = vals[1] * ADS_SCALE
            s.ads_ch[0] = vals[1]
            ...
            s.ads_mV[1] = vals[2] * ADS_SCALE
            if len(vals) >= 7:
                # p4_ch1/p4_ch2 = firmware-selected output (MAS-suppressed on
                # corrupt epochs). Plotted in place of the raw ADS leads.
                s.sel_ch[0] = vals[5]
                s.sel_ch[1] = vals[6]
                s.sel_mV[0] = vals[5] * ADS_SCALE
                s.sel_mV[1] = vals[6] * ADS_SCALE
                s.has_sel = True
            if len(vals) >= 22:
                s.p4_cycles = vals[21]   # CM7 Phase4 DSP cycles (DWT->CYCCNT)
            if len(vals) >= 23:
                s.m4_cycles = vals[22]   # CM4 epoch-classifier cycles (DWT->CYCCNT)
            return s
        if len(vals) == 4:
            s.ecg_corr = vals[1]
            s.ecg_mV   = s.ecg_corr * (VREF_MV / ADC_SPAN)
                ...
                while True:
                    ...
                    if line.startswith("t_us,"):
                        ...
                        if line not in (
                            ...
                            self._HDR_ADS1293_IMU_TS,
                            self._HDR_ADS1293_PHASE4_COMPACT,
                        ) and "p4_" not in line:
                        ...
                        self.mode_name = ("ECG_ONLY"    if line == self._HDR_ECG_ONLY
                                          ...
                                          else "ADS1293_IMU_TS" if line == self._HDR_ADS1293_IMU_TS
                                          else "ADS1293_PHASE4_COMPACT" if "p4_" in line
                                          else "ECG_IMU_ADS")
                        continue
                    ...
                    if sample is not None:
                        ...
                        if self.csv_header is None and sample.mode == "ADS1293_IMU_TS":
                            self.csv_header = self._HDR_ADS1293_IMU_TS
                            self.logger.set_csv_header(self.csv_header)
                            if not self._inferred_header_logged:
                                self._push_terminal("[GUI] Inferred ADS1293_IMU_TS from 24-column rows.")
                                self._inferred_header_logged = True
...
class ECGTab(QtWidgets.QWidget):
    ...
    def update(self, window, reader, logger, lps):
        ...
        show_sel = bool(ads) and ads[-1].has_sel
        if ads:
            xs = np.array([s.host_t - (now - WINDOW_S) for s in ads], dtype=np.float32)
            if show_sel:
                lead_i_raw = np.array([s.sel_mV[0] for s in ads], dtype=np.float32)
                lead_ii_raw = np.array([s.sel_mV[1] for s in ads], dtype=np.float32)
                lead_i_code = np.array([s.sel_ch[0] for s in ads], dtype=np.int32)
                lead_ii_code = np.array([s.sel_ch[1] for s in ads], dtype=np.int32)
            else:
        ...
        if ads:
            ...
            trace_src = "selected MAS output (p4_ch*)" if show_sel else "raw ADS leads"
            sel_line = (
                f" Selected  : I={last.sel_mV[0]:+.4f} mV  II={last.sel_mV[1]:+.4f} mV\n"
                if last.has_sel else ""
            )
            if last.p4_cycles or last.m4_cycles:
                p4_us = f" ({last.p4_cycles / reader.cm7_hz * 1e6:.1f} us)" if reader.cm7_hz else ""
                m4_us = f" ({last.m4_cycles / reader.cm4_hz * 1e6:.1f} us)" if reader.cm4_hz else ""
                perf_line = (
                    f" CM7 p4    : {last.p4_cycles} cyc{p4_us}\n"
                    f" CM4 class : {last.m4_cycles} cyc{m4_us}\n"
                )
            else:
                perf_line = ""
            data_txt = (
